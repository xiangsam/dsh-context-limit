 ...
 * the `dsh.compatibility` manifest field and the README.
 */
const TESTED_DSH_VERSIONS = ['0.1.5-alpha.2']
const SUPPORTED_DSH_RANGE = '>=0.1.1-rc.1 <0.2.0-0'
...
/** Whether a dsh version sits inside the supported window (the 0.1.x line). */
...
/**
 * Warn once when the running dsh is outside the supported window. Never
 * throws: a mismatched profile must still boot so the message stays readable.
 */
function warnOnUnsupportedDsh(ctx) {
  const version = runningDshVersion()
  if (version === undefined || isSupportedDshVersion(version)) return
  ctx.logger.error(
    `dsh-context-limit: unsupported dsh version ${version}; this release is verified against dsh `
    + `${TESTED_DSH_VERSIONS.join(' and ')} (supported range ${SUPPORTED_DSH_RANGE}). `
    + 'The composer slider may misbehave — install a matching dsh release.',
  )
}
